...
bootstrapBundleScript.setAttribute(
  "src",
  "https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/js/bootstrap.bundle.min.js"
);
watchScript.setAttribute("src", `${CDNlink}libraries/watch.js`);

const socketUrl = "https://socket-v2.marketrix.io/"; //"https://marketrix-soc.creative-hub.co/" // "http://192.168.1.76:8081"
      ...
      participant.on("stream-enabled", (stream) => {
        ...
        if (kind === "audio") {
          ...
          $(`#ai-${remoteId}`)
            ...
            .addClass("fa-solid fa-microphone");
        } else {
      ...
      participant.on("stream-disabled", (stream) => {
        ...
        if (kind === "audio") {
          ...
          $(`#ai-${remoteId}`)
            ...
            .removeClass("fa-solid fa-microphone");
        } else {
  ...
  toggle: {
    ...
    webCam: () => {
      if (meetingObj.isWebCamOn) {
        ...
        $(`#f-${meetingObj.meeting.localParticipant.id}`).css(
          "display",
          "none"
        );
        $("#webcam-icon")
          .addClass("fa-solid fa-video-slash")
          .removeClass("fas fa-video");
      } else {
        ...
        $("#webcam-icon")
          .removeClass("fa-solid fa-video-slash")
          .addClass("fas fa-video");
      }
...
const mouse = {
  ...
  show: () => {
    ...
    if (localId) {
      ...
      $(`#f-${localId}`).addClass(
        "moving-outer-frame local-moving-outer-frame"
      );
      $(`#v-${localId}`)
        .addClass("moving-video-frame")
        .removeClass("video-frame");
      $(`#f-${localId}`).css("margin-top", "-36px");
    }
    if (remoteId) {
      $(`#f-${remoteId}`).css("position", "absolute");
      $(`#f-${remoteId}`).addClass(
        "moving-outer-frame remote-moving-outer-frame"
      );
      $(`#v-${remoteId}`)
        .addClass("moving-video-frame")
        .removeClass("video-frame");
  ...
  hide: () => {
    ...
    if (localId) {
      ...
      $(`#f-${localId}`).css("left", "");
      $(`#f-${localId}`).css("top", "");
      $(`#f-${localId}`).removeClass(
        "moving-outer-frame local-moving-outer-frame"
      );
      $(`#v-${localId}`)
        .removeClass("moving-video-frame")
